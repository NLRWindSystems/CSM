...
class CostAndScalingModel:
    ...
    def __init__(
        ...
        self.parameter_inputs = self.parameters_all.difference(
            function_args_unordered.keys()
        )
        ...
        self.function_args = self.get_parameter_calculation_order(
            function_args_unordered
        )
    ...
    def calculate_parameter(
        ...
        **known_args,
        ...
        if (
            param_to_calculate in self.parameter_inputs
            and param_to_calculate not in known_args.keys()
        ):
            raise KeyError(
    ...
    def calculate_parameters(self, param_data):
        ...
        if isinstance(param_data, pd.DataFrame):
            ...
            raise SyntaxError(
                "input_data must be a dict (scalar) or DataFrame (vectorized)."
            )
        ...
        unnecessary_inputs = (
            ...
            .difference(self.parameter_inputs)
            .difference({"scenario"})
        )
    ...
    def get_parameter_calculation_order(
        self, function_args_unordered: dict
    ) -> OrderedDict:
        ...
        function_args_ordered = OrderedDict(
            {
                ...
                if len(
                    set(function_args_unordered[k]).difference(self.parameter_inputs)
                )
                == 0
            }
        )
        ...
        while bool(function_args_unordered):
            ...
            for name in tuple(function_args_unordered.keys()):
                ...
                if (
                    len(set(function_args_unordered[name]).difference(known_params))
                    == 0
                ):
