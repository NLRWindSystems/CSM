...
def get_value_with_fallback(rows, row_key, turbine_cols, preferred_idx):
...
def build_component_table(rows, turbine_row, model_row, turbine_name, model_variant):
    ...
    def val(row_key):
        return get_value_with_fallback(rows, row_key, turbine_cols, col_idx)
...
#        "Mass (kg)": val("pitch_system_mass"),
#        "Cost ($)": val("pitch_system_cost"),
#        "IRS Component": None,
#        "IRS Cost %": None,
#    })
    table.append({
...
def fmt_pct(value, irs_component):
    # If there is no value, leave the cell blank
    if value is None: 
        return ""
    # Since you already formatted the strings with "%" and "N/A" in the table, 
    # we just return the value exactly as you typed it.
    return str(value)
...
def main():
    ...
    parser.add_argument("--variant", type=str, default="Empirical2020", choices=MODEL_VARIANTS)
    ...
    # Automatically generate a clean CSV filename if --out isn't provided
    out_path = args.out
    if not out_path:
